...
<style>
    ...
    .calc-wrap {{ max-width: 350px; margin: 0 auto; }}
    .display {{ background: {theme['num']}; color: white; padding: 25px; font-size: 48px; text-align: right; border-radius: 20px 20px 0 0; }}
    .btn-grid {{ display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; padding: 10px; background: {theme['bg']}; border-radius: 0 0 20px 20px; }}
    .num-btn {{ background: {theme['num']}; color: white; }}
    .func-btn {{ background: {theme['func']}; color: black; }}
    .op-btn {{ background: {theme['op']}; color: black; }}
    .eq-btn {{ background: {theme['eq']}; color: white; }}
    .card {{ background: {theme['num']}; border-radius: 16px; padding: 15px; margin: 10px auto; max-width: 350px; }}
...
tab = st.radio("", ["🧮", "🏠", "🔄", "❤️"], horizontal=True, index=0, key="t")
tm = {'🧮':'calc','🏠':'loan','🔄':'unit','❤️':'health'}
...
if tm[tab] == 'calc':
    ...
    # 行1
    for i, (lbl, cls) in enumerate([('AC','func'),('±','func'),('%','func'),('÷','op')]):
        c1,c2,c3,c4 = st.columns(4)
        with [c1,c2,c3,c4][i]:
            if st.button(lbl, key=f"k{i}", use_container_width=True):
    ...
    # 行2-4
    for r in [[('7','num'),('8','num'),('9','num'),('×','op')],[('4','num'),('5','num'),('6','num'),('-','op')],[('1','num'),('2','num'),('3','num'),('+','op')]]:
        cols = st.columns(4)
        for i,(lbl,cls) in enumerate(r):
            with cols[i]:
                if st.button(lbl, key=f"k{lbl}", use_container_width=True):
                    st.session_state.expression+=lbl; st.session_state.display=st.session_state.expression; st.rerun()
    
    # 行5
    c1,c2,c3,c4 = st.columns(4)
    with c1:
        if st.button('0', key="k0", use_container_width=True): st.session_state.expression+='0'; st.session_state.display=st.session_state.expression; st.rerun()
    with c2:
        if st.button('.', key="kdot", use_container_width=True): st.session_state.expression+='.'; st.session_state.display=st.session_state.expression; st.rerun()
    with c4:
...
# 房贷
    ...
    c1,c2 = st.columns(2)
    with c1: amt = st.number_input("贷款(万)", 100.0, key="la")
    with c2: rt = st.number_input("利率(%)", 3.5, step=0.1, key="lr")
    yr = st.selectbox("年限", [5,10,15,20,25,30], index=3, key="ly")
    
    if st.button("计算", key="calc"):
...
# 换算
    ...
    ut = st.selectbox("类型", ["💱 货币","📏 长度","⚖️ 重量","🌡️ 温度"], key="ut")
    ...
    if ut == "💱 货币":
        ...
        with c1: amt = st.number_input("金额", 1.0, key="ca")
        ...
        c1,c2,c3 = st.columns(3)
        with c1: v = st.number_input("数值", 1.0, key="lv")
        with c2: f = st.selectbox("从", list(u.keys()), key="lf")
        with c3: t = st.selectbox("到", list(u.keys()), index=1, key="lt")
        ...
        c1,c2,c3 = st.columns(3)
        with c1: v = st.number_input("数值", 1.0, key="wv")
        with c2: f = st.selectbox("从", list(u.keys()), key="wf")
        with c3: t = st.selectbox("到", list(u.keys()), index=1, key="wt")
        st.markdown(f'<div style="background:{t_func}; padding:15px; border-radius:10px; text-align:center; margin-top:12px;"><div style="font-size:22px;">{v*u[f]/u[t]:.6f}</div></div>', unsafe_allow_html=True)
    
    elif ut == "🌡️ 温度":
        c1,c2 = st.columns(2)
        with c1: v = st.number_input("温度", 0.0, key="tv")
        with c2: f = st.selectbox("从", ["摄氏度","华氏度","开尔文"], key="tf")
        t = st.selectbox("到", ["摄氏度","华氏度","开尔文"], index=1, key="tt")
...
# 健康
    ...
    c1,c2 = st.columns(2)
    with c1: h = st.number_input("身高(cm)", 170, key="hh")
    with c2: w = st.number_input("体重(kg)", 65, key="hw")
    c3,c4 = st.columns(2)
    with c3: age = st.number_input("年龄", 30, key="ha")
    with c4: g = st.selectbox("性别", ["男","女"], key="hg")
    act = st.selectbox("运动", ["久坐(1.2)","轻度(1.375)","中度(1.55)","重度(1.725)"], key="hac")
    
    if st.button("计算", key="hc"):
        bmi = w/((h/100)**2)
